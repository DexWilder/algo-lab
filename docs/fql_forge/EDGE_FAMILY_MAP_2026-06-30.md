# Edge-Family Map (computed 2026-07-01 17:49 UTC) — from family_status.json x trial ledger
> Regenerate: `python3 research/forge_family_map.py`. Coverage=tested/(tested+untested). family-N from ledger lane.
> **Families: 21 | active (not killed): 14 | global trial-N: 1799 | drift flags: 0**

| family | status | tier(tested→applicable) | coverage | tested | untested | family-N | next untested expressions |
|---|---|---|---|---|---|---|---|
| gamma_dealer | FEASIBLE | none→T6 ⚠gap | 20% | 1 | 4 | 9 | chunked-OI loader; approx-GEX; GEX-regime pin |
| monthend_settlement | CLEAN_BUT_WEAK | T1→T3 ⚠gap | 25% | 1 | 3 | 7 | ZF/ZB month-end; settlement-1m revert; index-rebal |
| open_close_liquidity | CLEAN_BUT_WEAK | T3→T3 | 40% | 2 | 3 | 30 | settlement-window revert @T3; closing-imbalance @T3; liquidity-hole reversal @T3 |
| trend_momentum | CLEAN_BUT_WEAK | T2→T3 ⚠gap | 50% | 2 | 2 | 1679 | pooled vol-target trend; trend+carry combo |
| macro_event_drift | CLEAN_BUT_WEAK | T1→T3 ⚠gap | 50% | 2 | 2 | 3 | CPI drift; event-1m-path |
| fx_fixing_ratediv | UNDERTESTED | none→T3 ⚠gap | 0% | 0 | 2 | 9 | WMR 16:00 fix flow; policy-rate divergence |
| carry_commodity | SCREEN_PASS_RETAINED | T4→T4 | 88% | 7 | 1 | 10 | roll-window pressure |
| vol_risk_premium | CLEAN_BUT_WEAK | T5→T6 ⚠gap | 67% | 2 | 1 | 9 | true-curve VRP (paid) |
| inventory_eia | DATA_BLOCKED_CERT | none→T5 ⚠gap | 0% | 0 | 1 | 7 | EIA surprise hedge |
| expiry_opex | FEASIBLE | none→T6 ⚠gap | 0% | 0 | 1 | 9 | OPEX-gamma-pin (via gamma lane) |
| xasset_leadlag | UNDERTESTED | T2→T3 ⚠gap | 50% | 1 | 1 | 9 | pure latency lead-lag |
| regime_filters | UNDERTESTED | T5→T5 | 50% | 1 | 1 | 3 | regime overlay on a survivor (needs survivor) |
| execution_cost | ACTIVE_TOOL | T3→T3 | 50% | 1 | 1 | 9 | liquidity risk-filter |
| mean_reversion | TIER_INCOMPLETE | T2→T3 ⚠gap | 100% | 2 | 0 | 1679 | — |
...
| intraday_micro | SUBFAMILY_KILLED | T3→T6 ⚠gap | 50% | 1 | 1 | 30 | ES-NQ lead-lag latency |
