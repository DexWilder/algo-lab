...
- **MAE-guard / time-stop (`forge_cycle_2026-06-22g`, intra-trade path):** TIME-STOP/no-progress = KILL all 3 books (cuts net, no DD benefit). MAE-guard: only MNQ-stop_run_reversal interesting → hard MAE stop $250 cuts maxDD −4018→−2629 (−35%) for −5% net, monotone/stable across $150-400 band → **WATCH (DD-reduction overlay for the wired worst-DD book)** BUT fill approximated at exactly −L (OPTIMISTIC; real stop-slippage reduces benefit) + tradeoff → NOT confirmed/promotable, report-only. MNQ-ORB MAE non-monotone (no clean overlay); MGC-ORB MAE no benefit.
