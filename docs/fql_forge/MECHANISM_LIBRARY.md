...
# vol_low extension test (2026-06-25) → CONFIRMED MGC-specific (does NOT extend to M2K)
Tested vol_low on M2K-ORB: PF 1.278→1.264 (worse), net −$644, maxDD −1146→−1310 (worse), per-year flat-to-down → just trade-deletion, NO refinement. So MGC vol_low is GENUINELY ASSET-SPECIFIC (gold-only): gold's geopolitical vol spikes whipsaw ORB (excluding extreme-vol helps); small-cap equity lacks that pattern. Test PREVENTED over-generalizing the single-asset find into a false 'vol_low helps equity ORB' rule. MGC vol_low classification SHARPENED: asset-specific (MGC), not multi-asset.
...
# CRUDE RELATIVE-STRENGTH CLEAN-RETEST (2026-06-25) → KILL: confirmed 100% MCL artifact, branch CLOSED
XSMOM lb21, crude leg swapped MCL→USL→DBO→none (net 5bps). MCL(orig) Sharpe 1.38; **USL(clean) 0.63, DBO(clean) 0.63, NO-CRUDE 0.64** — USL=DBO=no-crude (conclusive, proxies AGREE). The entire MCL 1.38 'edge' was rollover/stitch artifact; clean crude adds NO signal (even slightly hurts H1/H2 + maxDD). Crude relative-strength / XSMOM cross-sectional = KILL, branch closed. Validates the clean-data-retest discipline: suspicious result on dirty data → retest on clean proxy (USL/DBO) → definitive answer. USL/DBO confirmed as clean crude RESEARCH proxies (execution stays MCL/CL).
...
1. **Roll-adjusted continuous futures (esp. crude/MCL)** — REACHABLE PARTIAL (2026-06-25 probe): roll-managed crude ETFs are far cleaner than our MCL continuous — **USL (12-mo laddered) = 3 days >8% vs MCL's 18**; DBO=5, USO=9. USL/DBO = reachable CLEAN crude PRICE proxy for daily-return signal research (resolves whether crude relative-strength is real or pure MCL artifact). Research-vs-execution gap remains: tradeable = MCL/CL; true roll-adjusted MCL continuous still class-C. NEXT: re-run crude relative-strength / XSMOM crude leg / commodity carry on clean USL. — back-adjusted at roll dates → fixes the rollover-gap artifacts ([[feedback_continuous_contract_rollover_artifacts]]) that contaminated XSMOM/S1; reopens crude relative-strength + commodity carry cleanly.
