    ...
    for e in matrix:
        ...
        if alloc:
            allocation_entries.append({
                "strategy_id": e["strategy_id"],
                "base_tier": alloc.get("base_tier_name", "OFF"),
                "target_tier": alloc.get("target_tier_name", "OFF"),
                "final_tier": alloc.get("final_tier_name", "OFF"),
                "sessions": alloc.get("sessions", {}),
                "adjustments": alloc.get("adjustments", []),
                "hard_restricted": alloc.get("hard_restricted", False),
            })
    report["sections"]["allocation_tiers"] = allocation_entries
...
def generate_markdown(report: dict) -> str:
    ...
    # 10. Allocation Tiers
    alloc_tiers = sections.get("allocation_tiers", [])
    if alloc_tiers:
        lines.append("## 10. Allocation Tiers")
        lines.append("")
        lines.append("| Strategy | Base | Target | Final | Morning | Midday | Afternoon | Adjustments |")
        lines.append("|---|---|---|---|---|---|---|---|")
        for a in alloc_tiers:
            sess = a.get("sessions", {})
            morn = sess.get("morning", {}).get("tier_name", "-")
            mid = sess.get("midday", {}).get("tier_name", "-")
            aftn = sess.get("afternoon", {}).get("tier_name", "-")
            adj_count = len(a.get("adjustments", []))
            adj_str = f"{adj_count} adj" if adj_count > 0 else "none"
            lines.append(
                f"| {a['strategy_id']} | {a['base_tier']} | {a['target_tier']} "
                f"| {a['final_tier']} | {morn} | {mid} | {aftn} | {adj_str} |"
            )
        lines.append("")
...
def print_terminal_report(report: dict):
    ...
    if alloc_tiers:
        print()
        print("  10. ALLOCATION TIERS")
        print(f"  {'-' * 55}")
        print(f"  {'Strategy':<28s} {'Base':<9s} {'Final':<9s} {'Morn':<6s} {'Mid':<6s} {'Aftn':<6s}")
        print(f"  {'-' * 66}")
        for a in alloc_tiers:
            sid = a["strategy_id"]
            if len(sid) > 27:
                sid = sid[:24] + "..."
            sess = a.get("sessions", {})
            morn = sess.get("morning", {}).get("tier_name", "-")[:5]
            mid = sess.get("midday", {}).get("tier_name", "-")[:5]
            aftn = sess.get("afternoon", {}).get("tier_name", "-")[:5]
            print(f"  {sid:<28s} {a['base_tier']:<9s} {a['final_tier']:<9s} {morn:<6s} {mid:<6s} {aftn:<6s}")
