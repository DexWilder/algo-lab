"""Cycle 2026-06-08j — volatility_regime_compound first batch.

Per operator decision #106 (Hybrid D-A-2). Tests the new vol-regime-compound
primitive across the operator-specified asset set: MGC, MCL, MES, MYM, MNQ.

PL default per #91. Strict gates per #102 + #100:
  - median >= $2 (not $1) buffer for upgrade consideration
  - PASS_STRESS at 2x cost + 2 ticks
  - max-year <= 50% concentration
  - Era3 median >= 0 (no current-regime weakness)
  - n >= 50 sample stability
...
# First batch — per operator: MGC/MCL/MES/MYM/MNQ (MNQ as benchmark only)
SPECS = [
    {"label": "VRC-MGC-Both-PL", "asset": "MGC", "entry": "volatility_regime_compound",
     "filter": "ema_slope", "exit": "profit_ladder", "mode": "both"},
    {"label": "VRC-MCL-Both-PL", "asset": "MCL", "entry": "volatility_regime_compound",
     "filter": "ema_slope", "exit": "profit_ladder", "mode": "both"},
    {"label": "VRC-MES-Both-PL", "asset": "MES", "entry": "volatility_regime_compound",
     "filter": "ema_slope", "exit": "profit_ladder", "mode": "both"},
    {"label": "VRC-MYM-Both-PL", "asset": "MYM", "entry": "volatility_regime_compound",
     "filter": "ema_slope", "exit": "profit_ladder", "mode": "both"},
    {"label": "VRC-MNQ-Both-PL", "asset": "MNQ", "entry": "volatility_regime_compound",
     "filter": "ema_slope", "exit": "profit_ladder", "mode": "both"},
]


def run():
    print(f"Cycle 2026-06-08j — volatility_regime_compound first batch ({len(SPECS)} candidates)", flush=True)
    print(f"Strict upgrade gates: median >= $2 AND PASS_STRESS AND max-yr <= 50% AND Era3 median >= 0", flush=True)
    ...
    # Strict upgrade check per #102 + #100 gates (median >= $2 raised buffer)
    upgrade_candidates = []
    observational = []
    kill = []
    for r in results:
    ...
    if upgrade_candidates:
        print("\nUPGRADE_CANDIDATE — all strict gates clear (requires family review before promotion):")
        for r in upgrade_candidates:
            m = r["metrics"]; ts = r["temporal_split"]
            era3_med_str = f"${ts['era3_median']:.2f}" if ts and ts.get('era3_median') is not None else 'n/a'
            print(f"  {r['spec']['label']}: PF={m['pf']:.3f} med=${m['median']:.2f} "
                  f"max-yr={m['max_year_share_pct']:.1f}% Era3-med={era3_med_str}", flush=True)
    else:
        print("\nNo candidate cleared all gates. Per #106 honest classification: report and move on.", flush=True)

    out = ROOT / "research" / "data" / "fql_forge" / "reports" / "forge_cycle_2026-06-08j.json"
    out.write_text(json.dumps({
        "date": date.today().isoformat(),
        "purpose": "volatility_regime_compound first batch (#106 Hybrid D-A-2)",
        "boundaries": "report-only Lane B",
        "strict_upgrade_gates": "median >= $2 AND PASS_STRESS AND max-yr <= 50% AND Era3 median >= 0 AND n >= 50",
        "tier_classification": {
