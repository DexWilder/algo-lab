"""Cycle 2026-06-08l — Source Mining Batch 1 (post-VRC-archive rotation).

Per operator decisions #110 (rotate immediately to Source Mining if VRC fails).

5 RUNNABLE_NOW candidates from source_mining_catalog_2026-06-08.md:
  1. bb_reversion × MES × hurst_stable_mr × PL × both    (FIRST hurst-filter use)
  2. bb_reversion × MNQ × hurst_stable_mr × PL × both    (SECOND hurst-filter use)
  3. vol_expansion × MES × session_morning × PL × both   (open-drive vol-expansion)
  4. prior_day_fade × MNQ × ema_slope × PL × both        (PDF on MNQ untested)
  5. vwap_continuation × MES × session_morning × PL × both (open-drive VWAP cont)

All use existing primitives — zero infrastructure build cost.
PL-default per #91. Strict gates per #100/#102/#106.

Notable: candidates 1+2 deploy hurst_stable_mr filter for FIRST TIME in
campaign. The filter was built in cycle 06-03 but never used in candidate
cycles — primitive-coverage waste identified during Source Mining catalog.
...
SPECS = [
    # Hurst-MR filter + bb_reversion on equity micros (FIRST hurst-filter deployment)
    {"label": "SM1-BB-MES-HurstMR-PL", "asset": "MES", "entry": "bb_reversion",
     "filter": "hurst_stable_mr", "exit": "profit_ladder", "mode": "both"},
    {"label": "SM1-BB-MNQ-HurstMR-PL", "asset": "MNQ", "entry": "bb_reversion",
     "filter": "hurst_stable_mr", "exit": "profit_ladder", "mode": "both"},

    # Open-drive mechanisms (session_morning on untested entries/assets)
    {"label": "SM1-VolExp-MES-MorningSession-PL", "asset": "MES", "entry": "vol_expansion",
     "filter": "session_morning", "exit": "profit_ladder", "mode": "both"},
    {"label": "SM1-VWAP-MES-MorningSession-PL", "asset": "MES", "entry": "vwap_continuation",
     "filter": "session_morning", "exit": "profit_ladder", "mode": "both"},

    # PDF on MNQ — untested asset for this entry
    {"label": "SM1-PDF-MNQ-PL", "asset": "MNQ", "entry": "prior_day_fade",
     "filter": "ema_slope", "exit": "profit_ladder", "mode": "both"},
]


def run():
    print(f"Cycle 2026-06-08l — Source Mining Batch 1 ({len(SPECS)} candidates)", flush=True)
    print(f"Post-VRC-archive rotation per #110. RUNNABLE_NOW only — existing primitives.", flush=True)
    print(f"FIRST deployment of hurst_stable_mr filter (built 06-03, never used in cycles).", flush=True)
    ...
    for i, spec in enumerate(SPECS, 1):
        ...
        print(
            f"  [{i}] {spec['label']:38s}: n={m['n']:5d} PF={m['pf']:.3f} "
    ...
    if upgrade_candidates:
        print("\nUPGRADE_CANDIDATE — strict gates clear (requires family review):")
    ...
    out = ROOT / "research" / "data" / "fql_forge" / "reports" / "forge_cycle_2026-06-08l.json"
    out.write_text(json.dumps({
        "date": date.today().isoformat(),
        "purpose": "Source Mining Batch 1 (post-VRC-archive rotation per #110)",
        "boundaries": "report-only Lane B; RUNNABLE_NOW only; no new primitive; no data unlock",
        "notable": "FIRST deployment of hurst_stable_mr filter (built 06-03, never used in cycles)",
        "tier_classification": {
