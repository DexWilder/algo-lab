"""Cycle 2026-06-08m — bb_keltner_squeeze first batch.

Per operator decision #111 (OK 111-A). New primitive deployed across 6 assets.
PL-default per #91. Strict gates per #100/#102/#106.

If 0 productive: archive primitive, immediately produce Source Mining Packet 2
(10-20 mechanisms, 5 RUNNABLE_NOW, 3 NEEDS_PRIMITIVE, 3 NEEDS_DATA).
...
SPECS = [
    {"label": "BBKC-MGC-Both-PL", "asset": "MGC", "entry": "bb_keltner_squeeze",
     "filter": "ema_slope", "exit": "profit_ladder", "mode": "both"},
    {"label": "BBKC-MCL-Both-PL", "asset": "MCL", "entry": "bb_keltner_squeeze",
     "filter": "ema_slope", "exit": "profit_ladder", "mode": "both"},
    {"label": "BBKC-MES-Both-PL", "asset": "MES", "entry": "bb_keltner_squeeze",
     "filter": "ema_slope", "exit": "profit_ladder", "mode": "both"},
    {"label": "BBKC-MYM-Both-PL", "asset": "MYM", "entry": "bb_keltner_squeeze",
     "filter": "ema_slope", "exit": "profit_ladder", "mode": "both"},
    {"label": "BBKC-MNQ-Both-PL", "asset": "MNQ", "entry": "bb_keltner_squeeze",
     "filter": "ema_slope", "exit": "profit_ladder", "mode": "both"},
    {"label": "BBKC-ZN-Both-PL", "asset": "ZN", "entry": "bb_keltner_squeeze",
     "filter": "ema_slope", "exit": "profit_ladder", "mode": "both"},
]


def run():
    print(f"Cycle 2026-06-08m — bb_keltner_squeeze first batch ({len(SPECS)} candidates)", flush=True)
    print(f"Strict gates: median >= $2 AND PASS_STRESS AND max-yr <= 50% AND Era3 median >= 0 AND n >= 50", flush=True)
    ...
    out = ROOT / "research" / "data" / "fql_forge" / "reports" / "forge_cycle_2026-06-08m.json"
    out.write_text(json.dumps({
        "date": date.today().isoformat(),
        "purpose": "bb_keltner_squeeze first batch (#111-A Hybrid D-A-3)",
        "boundaries": "report-only Lane B",
        "tier_classification": {
