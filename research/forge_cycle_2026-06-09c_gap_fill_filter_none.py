"""Cycle 2026-06-09c — gap_fill_trigger filter=none retry.

Per operator decision #118 (OK A — one bounded retry with filter=none).
Fair mechanism test after ema_slope filter mismatch in cycle 09b.

Same 6 assets: MES, MNQ, MYM, MGC, MCL, ZN.
filter = "none" (fade mechanism wants to trade against trend; ema_slope blocked it).
PL-default per #91. Strict gates per #100/#102/#106.

Classification after retry:
  - If all KILL with fair n: gap_fill → ARCHIVED (mechanism does not produce edge)
  - If thin FAIL_STRESS: OBSERVATIONAL + cost-sensitive inventory
  - If viable PASS_STRESS: queue family review / deep screen / audit
...
SPECS = [
    {"label": "GAPN-MES-Both-PL", "asset": "MES", "entry": "gap_fill_trigger",
     "filter": "none", "exit": "profit_ladder", "mode": "both"},
    {"label": "GAPN-MNQ-Both-PL", "asset": "MNQ", "entry": "gap_fill_trigger",
     "filter": "none", "exit": "profit_ladder", "mode": "both"},
    {"label": "GAPN-MYM-Both-PL", "asset": "MYM", "entry": "gap_fill_trigger",
     "filter": "none", "exit": "profit_ladder", "mode": "both"},
    {"label": "GAPN-MGC-Both-PL", "asset": "MGC", "entry": "gap_fill_trigger",
     "filter": "none", "exit": "profit_ladder", "mode": "both"},
    {"label": "GAPN-MCL-Both-PL", "asset": "MCL", "entry": "gap_fill_trigger",
     "filter": "none", "exit": "profit_ladder", "mode": "both"},
    {"label": "GAPN-ZN-Both-PL", "asset": "ZN", "entry": "gap_fill_trigger",
     "filter": "none", "exit": "profit_ladder", "mode": "both"},
]


def run():
    print(f"Cycle 2026-06-09c — gap_fill_trigger filter=none retry ({len(SPECS)} candidates)", flush=True)
    print(f"Per #118 OK A — fair mechanism test after ema_slope mismatch in cycle 09b.", flush=True)
    ...
    print(f"\nTotal signals across 6 assets: {total_signals}", flush=True)
    ...
    # Final classification per #118
    if total_signals < 30:
        final = "MECHANISM_BUILD_FAILED — primitive does not fire even without filter; archive."
    elif upgrade_candidates:
        final = "VIABLE SURVIVOR — queue family review / deep screen / audit per #118."
    elif observational:
        final = "thin FAIL_STRESS — OBSERVATIONAL; add to cost-sensitive inventory."
    else:
        final = "all KILL — gap_fill_trigger ARCHIVED (mechanism does not produce edge with fair test)."
    print(f"\nFINAL gap_fill CLASSIFICATION: {final}", flush=True)

    out = ROOT / "research" / "data" / "fql_forge" / "reports" / "forge_cycle_2026-06-09c.json"
    out.write_text(json.dumps({
        "date": date.today().isoformat(),
        "purpose": "gap_fill_trigger filter=none retry (#118 fair mechanism test)",
        "boundaries": "report-only Lane B; ONE retry only",
        "param_change": "filter: ema_slope → none",
