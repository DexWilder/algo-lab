"""Cycle 2026-06-09d — stop_run_reversal first batch.

Per operator decision #118 chain + SMP-3. New primitive deployed across 5
core assets. PL-default per #91. Filter=none per filter pre-flight rule #120
(reversal thesis is structurally incompatible with ema_slope).
...
SPECS = [
    {"label": "SRR-MGC-Both-PL", "asset": "MGC", "entry": "stop_run_reversal",
     "filter": "none", "exit": "profit_ladder", "mode": "both"},
    {"label": "SRR-MCL-Both-PL", "asset": "MCL", "entry": "stop_run_reversal",
     "filter": "none", "exit": "profit_ladder", "mode": "both"},
    {"label": "SRR-MES-Both-PL", "asset": "MES", "entry": "stop_run_reversal",
     "filter": "none", "exit": "profit_ladder", "mode": "both"},
    {"label": "SRR-MYM-Both-PL", "asset": "MYM", "entry": "stop_run_reversal",
     "filter": "none", "exit": "profit_ladder", "mode": "both"},
    {"label": "SRR-MNQ-Both-PL", "asset": "MNQ", "entry": "stop_run_reversal",
     "filter": "none", "exit": "profit_ladder", "mode": "both"},
]


def run():
    print(f"Cycle 2026-06-09d — stop_run_reversal first batch ({len(SPECS)} candidates)", flush=True)
    print(f"Filter pre-flight applied: reversal thesis → filter=none (ema_slope contradicts).", flush=True)
    ...
    out = ROOT / "research" / "data" / "fql_forge" / "reports" / "forge_cycle_2026-06-09d.json"
    out.write_text(json.dumps({
        "date": date.today().isoformat(),
        "purpose": "stop_run_reversal first batch (#118 chain + SMP-3)",
        "boundaries": "report-only Lane B",
        "filter_pre_flight": "reversal thesis → filter=none (per #120 rule)",
        "tier_classification": {
