"""Cycle 2026-06-10a — abnormal_range_followup first batch.

Per operator decision #127. New primitive deployed across 6 assets × 2 modes.
PL-default per #91. Filter=none per filter pre-flight rule #120 (bidirectional
thesis — both continuation and fade modes tested).

Strict gates per #100/#102/#106:
  - median >= $2
  - PF >= 1.15 minimum cheap-screen
  - max-yr <= 50%
  - Era 3 median >= 0
  - PASS_STRESS on every WATCH
...
def make_runner(spec):
    ...
    def runner(commission_mult, slippage_mult):
        ...
        mode = spec.get("backtest_mode", "both")  # NB: "mode" param is for entry; use backtest_mode for run_backtest mode
        return run_backtest(
...
def make_spec(asset, mode_name):
    return {
        "label": f"ARF-{asset}-{mode_name[:4]}-PL",
        "asset": asset,
...
SPECS = []
for asset in ["MES", "MNQ", "MGC", "MCL", "MYM", "ZN"]:
    for mode_name in ["continuation", "fade"]:
        SPECS.append(make_spec(asset, mode_name))


def run():
    print(f"Cycle 2026-06-10a — abnormal_range_followup first batch ({len(SPECS)} candidates)", flush=True)
    print(f"6 assets × 2 modes (continuation, fade). filter=none per #120 pre-flight.", flush=True)
    ...
    for i, spec in enumerate(SPECS, 1):
        ...
        print(
            f"  [{i}] {spec['label']:30s}: n={m['n']:5d} PF={m['pf']:.3f} "
    ...
    out = ROOT / "research" / "data" / "fql_forge" / "reports" / "forge_cycle_2026-06-10a.json"
    out.write_text(json.dumps({
        "date": date.today().isoformat(),
        "purpose": "abnormal_range_followup first batch (#127)",
        "boundaries": "report-only Lane B",
        "filter_pre_flight": "bidirectional thesis → filter=none per #120",
        "tier_classification": {
