"""Cycle 2026-06-10b — abnormal_range_followup bug-fixed retry.

Per operator decision #130 (OK A). Same 12 candidates as cycle 10a, but
crossbreeding_engine now uses entry_ok instead of in_session for the
session-open detection (fix for the entry_ok bug discovered post-10a).
...
def make_runner(spec):
    ...
    def runner(commission_mult, slippage_mult):
        ...
        mode = spec.get("backtest_mode", "both")
        return run_backtest(
...
def make_spec(asset, mode_name):
    return {
        "label": f"ARF2-{asset}-{mode_name[:4]}-PL",
        "asset": asset,
...
def run():
    print(f"Cycle 2026-06-10b — abnormal_range BUG-FIXED RETRY ({len(SPECS)} candidates)", flush=True)
    print(f"Fix: entry uses entry_ok instead of in_session for session-open detection.", flush=True)
    print(f"Boundaries: report-only Lane B. ONE retry only.\n", flush=True)
    ...
    if upgrade_candidates:
        print("\nUPGRADE_CANDIDATE — strict gates clear:")
    ...
    out = ROOT / "research" / "data" / "fql_forge" / "reports" / "forge_cycle_2026-06-10b.json"
    out.write_text(json.dumps({
        "date": date.today().isoformat(),
        "purpose": "abnormal_range_followup bug-fixed retry (#130)",
        "boundaries": "report-only Lane B; ONE retry only",
        "fix": "crossbreeding_engine entry uses entry_ok instead of in_session",
        "tier_classification": {
