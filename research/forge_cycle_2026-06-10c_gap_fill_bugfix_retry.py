"""Cycle 2026-06-10c — gap_fill_trigger bug-fixed retry.
...
def make_spec(asset):
    return {
        "label": f"GAPN2-{asset}-Both-PL",
        "asset": asset,
        "entry": "gap_fill_trigger",
        "filter": "none",
        "exit": "profit_ladder",
        "backtest_mode": "both",
    }
...
SPECS = [make_spec(a) for a in ["MES", "MNQ", "MGC", "MCL", "MYM", "ZN"]]


def run():
    print(f"Cycle 2026-06-10c — gap_fill BUG-FIXED RETRY ({len(SPECS)} candidates)", flush=True)
    ...
    out = ROOT / "research" / "data" / "fql_forge" / "reports" / "forge_cycle_2026-06-10c.json"
    out.write_text(json.dumps({
        "date": date.today().isoformat(),
        "purpose": "gap_fill_trigger bug-fixed retry (#131)",
        "boundaries": "report-only Lane B; ONE retry only",
