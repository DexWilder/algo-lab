"""Cycle 2026-06-12f — Test opening_drive_exhaustion primitive on MNQ/MES.

Per operator #190 C continuing Lane B in parallel.

New primitive: entry_opening_drive_exhaustion (added to crossbreeding_engine.py).

Mechanism: trade reversals of overextended opening drives (mean-reversion).

V1 workhorse hard gates + family review.
...
def run():
    print("Cycle 2026-06-12f — opening_drive_exhaustion primitive test on MNQ/MES\n", flush=True)
    ...
    results = []
    for asset in ["MNQ", "MES"]:
        label = f"WH-{asset}-opening_drive_exhaustion-ema_slope-PL"
        t0 = time.time()
        try:
            m, trades = _run(asset, "opening_drive_exhaustion", "profit_ladder", "ema_slope", label)
            v = _classify(m)
            daily = daily_metrics(trades, sessions[asset])
            ts = None; stress_m = None; v1_gates = None; v1_verdict = None
            if v == "ESCALATE_TO_V1_AUDIT":
                stress_m, _ = _run(asset, "opening_drive_exhaustion", "profit_ladder", "ema_slope",
        ...
        print(
            f"  {label:60s}: n={m['n']:5d} PF={m['pf']:.3f} "
    ...
    for r in results:
        ...
        _, t_orb_p = _run("MNQ", "orb_breakout", "profit_ladder", "ema_slope", "ref")
        ...
        _, t_orb_f = _run("MNQ", "orb_failure_reversal", "profit_ladder", "ema_slope", "ref")
        fam = {
    ...
    out = ROOT / "research" / "data" / "fql_forge" / "reports" / "forge_cycle_2026-06-12f_opening_drive_exhaustion_test.json"
    out.write_text(json.dumps({
        "date": date.today().isoformat(),
        "purpose": "Test opening_drive_exhaustion primitive (NEEDS_PRIMITIVE #4) on MNQ/MES",
