"""Cycle 2026-06-12g — Test afternoon_continuation + afternoon_reversion primitives.

Per operator #193 A: continue Lane B nonstop to NEEDS_PRIMITIVE #5.

Two new primitives:
  - entry_afternoon_continuation: morning bias continues post-13:00
  - entry_afternoon_reversion: morning bias reverses post-13:00

Test matrix: 2 primitives × 2 assets × ema_slope × profit_ladder = 4 candidates.
...
def run():
    print("Cycle 2026-06-12g — afternoon_continuation + afternoon_reversion test\n", flush=True)
    ...
    t_start = time.time()
    results = []
    for entry in ["afternoon_continuation", "afternoon_reversion"]:
        for asset in ["MNQ", "MES"]:
            ...
            print(
                f"  {label:62s}: n={m['n']:5d} PF={m['pf']:.3f} "
    ...
    print(f"\n--- Family review on any PAPER_PACKET_CANDIDATE ---", flush=True)
    for r in results:
    ...
    out = ROOT / "research" / "data" / "fql_forge" / "reports" / "forge_cycle_2026-06-12g_afternoon_continuation_reversion.json"
    out.write_text(json.dumps({
        "date": date.today().isoformat(),
        "purpose": "Test afternoon_continuation + afternoon_reversion primitives (NEEDS_PRIMITIVE #5)",
