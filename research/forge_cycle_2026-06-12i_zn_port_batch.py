"""Cycle 2026-06-12i — ZN port batch: 3 GREEN MNQ workhorses → ZN as-is.

Per operator #197 B authorization (post-Test-1):
  - port WH-MNQ-stop_run_reversal to ZN
  - port WH-MNQ-range_compression_break to ZN
  - port WH-MNQ-first_impulse_pullback to ZN

Rules per operator:
  - AS-IS transfer first, no threshold tuning before baseline result
  - report-only, no registry/scheduler/portfolio mutation
  - classify honestly as PASS/WATCH/KILL
  - if transfer fails: archive the direct port thesis
  - if transfer shows life: only THEN write rates-specific refinement thesis
    before any threshold adaptation
...
def run():
    print("Cycle 2026-06-12i — ZN PORT BATCH: 3 GREEN MNQ workhorses → ZN as-is\n", flush=True)
    print("Per operator #197 B. No tuning. Honest transfer test.\n", flush=True)
    ...
    zn_sessions = pd.to_datetime(rth["datetime"]).dt.date.nunique()
    print(f"ZN RTH sessions: {zn_sessions}\n", flush=True)

    PORTS = ["stop_run_reversal", "range_compression_break", "first_impulse_pullback"]

    t_start = time.time()
    results = []
    for entry in PORTS:
        label = f"WH-ZN-{entry}-ema_slope-PL"
        t0 = time.time()
        try:
            m, trades = _run("ZN", entry, "profit_ladder", "ema_slope", label)
            v = _classify(m)
            daily = daily_metrics(trades, zn_sessions)
            ts = None; stress_m = None; v1_gates = None; v1_verdict = None
            if v == "ESCALATE_TO_V1_AUDIT":
                stress_m, _ = _run("ZN", entry, "profit_ladder", "ema_slope",
        ...
        results.append({
            "label": label, "asset": "ZN", "entry": entry,
    ...
    print(f"\n--- ZN PAPER_PACKET_CANDIDATE family review (vs MNQ inventory) ---", flush=True)
    for r in results:
        if "error" in r or r.get("v1_verdict") != "PAPER_PACKET_CANDIDATE": continue
        # vs MNQ originals
        _, t_mnq_stop = _run("MNQ", "stop_run_reversal", "profit_ladder", "ema_slope", "ref")
        _, t_mnq_range = _run("MNQ", "range_compression_break", "profit_ladder", "ema_slope", "ref")
        _, t_mnq_fip = _run("MNQ", "first_impulse_pullback", "profit_ladder", "ema_slope", "ref")
        _, t_mnq_orb = _run("MNQ", "orb_breakout", "profit_ladder", "ema_slope", "ref")
        fam = {
            "vs_MNQ_stop_run": family_corr(r["trades_for_family"], t_mnq_stop),
            "vs_MNQ_range_compression": family_corr(r["trades_for_family"], t_mnq_range),
            "vs_MNQ_first_impulse": family_corr(r["trades_for_family"], t_mnq_fip),
            "vs_XB_ORB_MNQ": family_corr(r["trades_for_family"], t_mnq_orb),
        }
    ...
    print(f"\nZN PORT V1 tier: PAPER_PACKET_CANDIDATE={len(paper)} WATCH={len(watch)} ARCHIVED={len(archived)}", flush=True)

    print(f"\nTotal: {time.time() - t_start:.0f}s", flush=True)

    out = ROOT / "research" / "data" / "fql_forge" / "reports" / "forge_cycle_2026-06-12i_zn_port_batch.json"
    out.write_text(json.dumps({
        "date": date.today().isoformat(),
        "purpose": "ZN port batch (3 GREEN MNQ workhorses ported as-is) per #197 B",
        "boundary": "as-is port, no tuning, honest transfer test",
