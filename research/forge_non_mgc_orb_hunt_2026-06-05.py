"""Non-MGC-ORB Core Daily Hunt + prop-stress on WATCH.

Per operator approvals 2026-06-05 (#79 direction ALL; #80 feature-cache built).
8 candidates targeting operator priority list:
  Priority 1: MCL ORB SHORT-only; MGC PriorDayBreak SHORT
  Priority 2: MES PB directional/fixed-ratio; ZN PB directional/fixed-ratio

Feature cache makes prop-stress cheap (1st backtest per asset ~80s; subsequent
~5-10s). Apply prop-stress upfront on any WATCH.
"""
...
SPECS = [
    # Priority 1
    {"label": "P1-MCL-ORB-Short-PL", "asset": "MCL", "entry": "orb_breakout",
     "filter": "ema_slope", "exit": "profit_ladder", "mode": "short"},
    {"label": "P1-MCL-ORB-Short-FR2", "asset": "MCL", "entry": "orb_breakout",
     "filter": "ema_slope", "exit": "fixed_ratio", "params": {"ratio": 2.0},
     "mode": "short"},
    {"label": "P1-MGC-PriorDayBreak-Short", "asset": "MGC", "entry": "prior_day_break",
     "filter": "ema_slope", "exit": "profit_ladder", "mode": "short"},

    # Priority 2: MES PB
    {"label": "P2-MES-PB-Long-PL", "asset": "MES", "entry": "pb_pullback",
     "filter": "ema_slope", "exit": "profit_ladder", "mode": "long"},
    {"label": "P2-MES-PB-Short-PL", "asset": "MES", "entry": "pb_pullback",
     "filter": "ema_slope", "exit": "profit_ladder", "mode": "short"},
    {"label": "P2-MES-PB-Both-FR2", "asset": "MES", "entry": "pb_pullback",
     "filter": "ema_slope", "exit": "fixed_ratio", "params": {"ratio": 2.0},
     "mode": "both"},

    # Priority 2: ZN PB
    {"label": "P2-ZN-PB-Long-PL", "asset": "ZN", "entry": "pb_pullback",
     "filter": "ema_slope", "exit": "profit_ladder", "mode": "long"},
    {"label": "P2-ZN-PB-Short-PL", "asset": "ZN", "entry": "pb_pullback",
     "filter": "ema_slope", "exit": "profit_ladder", "mode": "short"},
]


def run():
    print(f"Non-MGC-ORB hunt — {len(SPECS)} candidates with prop-stress on WATCH", flush=True)
    ...
    for i, spec in enumerate(SPECS, 1):
        ...
        stress_str = ""
        if stress:
            stress_str = f" stress={stress['verdict'].split()[0]}"
        print(
    ...
    for r in results:
        ...
        if "KILL" in v or "ARCHITECTURAL_REJECT" in v:
            ...
            observational.append(r)
    print(f"\nTier classification: PAPER_PACKET={len(paper_packet)} PORTFOLIO_COMPLEMENT={len(portfolio_complement)} "
    ...
    out = ROOT / "research" / "data" / "fql_forge" / "reports" / f"forge_non_mgc_orb_hunt_2026-06-05.json"
    out.write_text(json.dumps({
        "date": date.today().isoformat(),
        "approval": "OK direction all (#79); feature-cache shipped (#80)",
        "tier_classification": {
